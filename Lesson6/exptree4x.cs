...
interface exprvisitor2 : exprvisitor
{
...
class modnode : expr
{
    ...
    //public object accept(exprvisitor2 v) { return v.visit(this); }
}
...
class evalvisitor2 : evalvisitor, exprvisitor2, exprvisitor
{
...
class sizecounter : exprvisitor, exprvisitor2
{
    ...
    public object visit(plusnode e) {
        return (int)e.left.accept(this) + (int)e.right.accept(this) + 1;
    }
  ...
  public static void Main()
  {
    //      expr A = new plusnode(new timesnode(new intnode(2),new intnode(3)),
      //                    new plusnode(new intnode(1),new intnode(4)));
        ...
        expr B = new plusnode(new timesnode(new intnode(2), new intnode(3)),
                             new modnode(new intnode(1),new intnode(4)));

           printvisitor printer = new printvisitor2();
    evalvisitor evaluator = new evalvisitor2();

        sizecounter sizeoftree = new sizecounter();
  //  A.accept(printer);
  //  int v = (int)A.accept(evaluator); // cast from object to int needed   
 //   Console.WriteLine("\nvalue is "+ v );
      
        B.accept(printer);
         int b = (int)B.accept(evaluator);
        Console.WriteLine("\nvalue is " + b);

        B.accept(sizeoftree);
        int sofb = (int)B.accept(sizeoftree);
        Console.WriteLine("the size of this three is " + sofb);
    } // main
}
